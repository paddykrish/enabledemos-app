    ...
    $scope.getData = function(locationId, buildingId) {
        $http.get('data/location_' + locationId + '_bldg_' + buildingId + '.json')
            .then(function(response) {
                ...
                //console.log(response.data.riskExposure[0].weather);
                $scope.weatherResults = response.data.riskExposure[0].weather;
                $scope.fireResults = response.data.riskExposure[1].fire;
                $scope.crimeResults = response.data.riskExposure[2].crime;
                $scope.hailResults = response.data.riskExposure[3].hail;
